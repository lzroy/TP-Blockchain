...
def op_checksig(stack, z):
    ...
    DER_sig = stack.pop()[:-1]
    try:
        s256 = S256Point.parse(SEC_pubkey)
        sig = Signature.parse(DER_sig)
    except Exception as e:
        LOGGER.info(f"Problem occured while serializing stack value in order to checksig {e}")
        return False
    stack.append(encode_num(1)) if s256.verify(z, sig) else stack.append(encode_num(0))
    return True
